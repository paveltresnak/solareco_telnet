...
SENSORS = [
    SolarecoSensorConfig('relay', None, None, 
        lambda data: re.findall('AC+\d', data)[0][2:] if re.search('AC+\d', data) else (re.findall('R:+\d', data)[0][2:] if re.search('R:+\d', data) else None),
        "mdi:electric-switch"),
    SolarecoSensorConfig('fan', None, None, 
        lambda data: re.findall('F:+\d', data)[0][2:] if re.search('F:+\d', data) else None,
        "mdi:fan"),
    SolarecoSensorConfig('required_voltage', UnitOfElectricPotential.VOLT, None, 
        lambda data: int(re.findall('U:(\d+)\s', data)[0]) if re.search('U:(\d+)\s', data) else None,
        "mdi:alpha-v-circle-outline"),
    SolarecoSensorConfig('voltage', UnitOfElectricPotential.VOLT, SensorDeviceClass.VOLTAGE, 
        lambda data: int(re.findall('(\d+)V', data)[0]) if re.search('(\d+)V', data) else None,
        "mdi:alpha-v-circle-outline"),
    SolarecoSensorConfig('current', UnitOfElectricCurrent.MILLIAMPERE, SensorDeviceClass.CURRENT, 
        lambda data: int(re.findall('(\d+)mA', data)[0]) if re.search('(\d+)mA', data) else None,
        "mdi:current-dc"),
    SolarecoSensorConfig('power', UnitOfPower.WATT, SensorDeviceClass.POWER, 
        lambda data: int(re.findall('(\d+)W', data)[0]) if re.search('(\d+)W', data) else None,
        "mdi:alpha-w-circle-outline"),
    SolarecoSensorConfig('frequency', UnitOfFrequency.HERTZ, SensorDeviceClass.FREQUENCY, 
        lambda data: int(re.findall('(\d+)Hz', data)[0]) if re.search('(\d+)Hz', data) else None,
        "mdi:sine-wave"),
    SolarecoSensorConfig('cooler_temperature', UnitOfTemperature.CELSIUS, SensorDeviceClass.TEMPERATURE, 
        lambda data: int(re.findall('(\d+)C', data)[0]) if re.search('(\d+)C', data) else None,
        "mdi:thermometer"),
    SolarecoSensorConfig('boiler_temperature', UnitOfTemperature.CELSIUS, SensorDeviceClass.TEMPERATURE, 
        lambda data: int(re.findall('(\d+):\d+C', data)[0]) if re.search('(\d+):\d+C', data) else None,
        "mdi:water-boiler"),
    SolarecoSensorConfig('pulse_width', UnitOfTime.MICROSECONDS, None, 
        lambda data: int(re.findall('(\d+)us', data)[0]) if re.search('(\d+)us', data) else None,
        "mdi:pulse"),
    SolarecoSensorConfig('total_energy', UnitOfEnergy.WATT_HOUR, SensorDeviceClass.ENERGY, 
        lambda data: int(re.findall('\d+kWh\s(\d+)Wh', data)[0]) if re.search('\d+kWh\s\d+Wh', data) else None,
        "mdi:lightning-bolt-circle",
        SensorStateClass.TOTAL_INCREASING),
    SolarecoSensorConfig('day_energy', UnitOfEnergy.WATT_HOUR, SensorDeviceClass.ENERGY, 
        lambda data: int(re.findall('(\d+)Wh', data)[-1]) if re.search('(\d+)Wh', data) else None,
        "mdi:solar-power",
        SensorStateClass.TOTAL_INCREASING),
]
